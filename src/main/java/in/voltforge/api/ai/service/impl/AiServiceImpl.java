...
import in.voltforge.api.ai.engine.WiringRuleEngine;
...
/**
 * AI Service implementation using:
 * 1. WiringRuleEngine (primary) — instant, deterministic wiring suggestions
 * 2. Ollama (fallback) — for code generation, chat, and complex queries
 */
@Slf4j
@Service
public class AiServiceImpl implements AiService {
    ...
    private final WiringRuleEngine wiringRuleEngine;

    public AiServiceImpl(@Qualifier("ollamaWebClient") WebClient ollamaWebClient,
                         OllamaConfig ollamaConfig,
                         ObjectMapper objectMapper,
                         WiringRuleEngine wiringRuleEngine) {
        this.ollamaWebClient = ollamaWebClient;
        this.ollamaConfig = ollamaConfig;
        this.objectMapper = objectMapper;
        this.wiringRuleEngine = wiringRuleEngine;
    }
    ...
    public AiGenerateResponse suggestWiring(AiGenerateRequest request) {
        ...
        // ── Step 1: Parse component info from the prompt ──
        ...
        // ── Step 2: Use the rule-based engine (instant, no external AI) ──
        List<AiWireSuggestion> suggestions = wiringRuleEngine.generateWiringSuggestions(components, boardType);

        if (!suggestions.isEmpty()) {
            log.info("Rule engine generated {} wire suggestions", suggestions.size());
            return AiGenerateResponse.builder()
                    .status("SUCCESS")
                    .message("Wiring suggestions generated by VoltForge AI (rule engine). " + suggestions.size() + " connections suggested.")
                    .wireSuggestions(suggestions)
                    .build();
        }

        // ── Step 3: Fallback to Ollama if rule engine couldn't help ──
        log.info("Rule engine returned no suggestions, trying Ollama fallback...");
        try {
            ...
            suggestions = extractWireSuggestions(aiResponse);
        } catch (Exception e) {
            log.warn("Ollama fallback failed: {}", e.getMessage());
        }
        ...
        if (suggestions.isEmpty()) {
            suggestions.add(AiWireSuggestion.builder()
                    ...
                    .color("#FF5722").description("Default: Connect D13 to LED")
                    .build());
        }

        return AiGenerateResponse.builder()
                .status("SUCCESS")
                .message("Wiring suggestions generated.")
                .wireSuggestions(suggestions)
                .build();
    }

    /**
     * Parse component information from the prompt text sent by the frontend.
     * The frontend sends lines like:
     *   "- comp_id_123: Arduino Uno (ARDUINO_UNO) pins: d0/D0/RX, d1/D1/TX, ..."
     */
    ...
    private List<Map<String, Object>> parseComponentsFromPrompt(String prompt) {
        ...
        for (String line : prompt.split("\\n")) {
            Matcher m = compPattern.matcher(line.trim());
            if (m.find()) {
        ...
        log.debug("Parsed {} components from prompt", components.size());
        return components;
    }
